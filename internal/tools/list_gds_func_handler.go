...
// Used when obtaining a list containing name and description of GDS functions
type GDSFunctionSummary struct {
...
// LLMGDSSummaryResponseWrapper is deprecated - use LLMResponseWrapper[[]GDSFunctionSummary] instead
// This type is kept for backward compatibility but new code should use the standardized response format
type LLMGDSSummaryResponseWrapper struct {
	Type         string
	Summary      string
	Data         []GDSFunctionSummary
...
// MapGDSFunctionSummaryList converts raw Neo4j records to clean structs
func MapGDSFunctionSummaryList(records []*neo4j.Record) ([]GDSFunctionSummary, error) {
	functions := make([]GDSFunctionSummary, 0, len(records))

	for _, record := range records {
		function, err := mapSingleGDSFunctionSummary(record)
		if err != nil {
			return nil, fmt.Errorf("failed to map function: %w", err)
		}
		functions = append(functions, function)
...
// mapSingleGDSFunction extracts the summary of a  GDS Function summary from a Neo4j record
func mapSingleGDSFunctionSummary(record *neo4j.Record) (GDSFunctionSummary, error) {
	name, _ := record.Get("name")
	description, _ := record.Get("description")

	function := GDSFunctionSummary{
		Name:        getString(name),
...
func SummaryOfGDSFunctionsHandler(deps *ToolDependencies) func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		return handleSummaryOfGDSFunctions(ctx, request, deps.DBService, deps.Config)
	}
}

func handleSummaryOfGDSFunctions(ctx context.Context, request mcp.CallToolRequest, dbService database.Service, config *config.Config) (*mcp.CallToolResult, error) {

	// Gets the name and description of available GDS functions
	...
	functions, err := MapGDSFunctionSummaryList(records)
	if err != nil {
		log.Fatal(err)
	}
	...
	LLMResponse := CreateLLMResponse(
		SummaryGDSFunctionsListed,
		functions,
		NextStepsAfterGDSFunctionList...,
	)
