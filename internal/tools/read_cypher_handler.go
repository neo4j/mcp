...
// ReadCypherHandler handles the read-cypher tool requests
func ReadCypherHandler(deps *ToolDependencies) func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		return handleReadCypher(ctx, request, deps.Driver, deps.Config)
	}
}

// handleReadCypher handles the read-cypher tool requests
func handleReadCypher(ctx context.Context, request mcp.CallToolRequest, driver *neo4j.DriverWithContext, config *config.Config) (*mcp.CallToolResult, error) {
	...
	records, err := database.ExecuteReadQuery(ctx, driver, Query, Params, config.Database)
	if err != nil {
